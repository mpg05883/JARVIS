...
def main(
    ...
    if dependency_type == "resource":
        ...
        ), "Resource dependency type only support data_huggingface and data_multimedia"

    arguments = locals()
    # Use HTTPS for external APIs, HTTP for local servers
    if api_addr in [
        "api.openai.com",
        "api.anthropic.com",
            ...
            if data["id"] in demos_id:
                if dependency_type == "temporal":
                    demo = {
                        ...
                        "result": {
                            "task_steps": data.get(
                                "task_steps", data.get("tool_steps", "")
                            ),
                            "task_nodes": data.get(
                                "task_nodes", data.get("tool_nodes", "")
                            ),
                            "task_links": data.get(
                                "task_links", data.get("tool_links", "")
                            ),
                        },
                    }
                else:
    ...
    try:
        ...
        if isinstance(content, list) and len(content):
            ...
            for c in content:
                for k, v in c.items():
                    (
                        merge_content[k].extend(v)
                        if k in merge_content
                        else merge_content.update({k: v})
                    )
        return content
        ...
        if reformat:
            ...
            if log_detail:
                logger.info(
                    f"{emoji.emojize(':warning:')}  #id {id} Illegal JSON format: {content}"
                )
                logger.info(
                    f"{emoji.emojize(':sparkles:')} #id {id} Detected illegal JSON format, try to reformat by {payload['model']}..."
                )
